...
type AdminWriterQueries interface {
	Publish(payload models.WriterPayload) error
}

type SQLAdminWriterQueries struct {
	db dbpkg.DBTX
}

func NewSQLAdminWriterQueries(db dbpkg.DBTX) *SQLAdminWriterQueries {
	return &SQLAdminWriterQueries{db: db}
}
...
// on update).
func (q *SQLAdminWriterQueries) Publish(payload models.WriterPayload) error {
	visible := true
